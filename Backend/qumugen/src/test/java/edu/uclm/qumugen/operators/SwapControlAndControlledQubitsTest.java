...
class SwapControlAndControlledQubitsTest {

    @BeforeAll
    static void setUp() throws Exception {
        Gate.loadGates(new SwapControlAndControlledQubitsTest());
        Operator.loadOperators(new SwapControlAndControlledQubitsTest());
    }

    @Test
    void testApply() {
        SwapControlAndControlledQubits op = new SwapControlAndControlledQubits();
        op.setName("SwapControlAndControlledQubits");

        String json = "{\"init\":[\"0\",\"0\"],\"cols\":[[\"•\",\"X\"]]}";
