...
public class ChangeControlQubitDown extends Operator {
	...
	protected List<Integer> getApplicableGates(List<Integer> rowsToMutate, QColumn column) {
		...
		// Si queremos mutar las de Toffoli, este if se puede quitar y se agregarían
		// todos los qubits de control de la columna
		if (column.getControlGates().size() == 1) {
			for (int i = 0; i < column.getGates().size(); i++) {
				...
				if (gate.isControl() && (!DownGate.isDuplicable() && !DownGate.isControl())) {
					result.add(i);
				}
			}
		}
	...
	protected void apply(Circuit circuit, int column, int row, List<Circuit> mutants) {
		...
		for (int i = 1; i < (circuit.getQubits() - row); i++) {
			...
			if (!DownGate.isDuplicable() && !DownGate.isControl() && (row + 1 <= circuit.getQubits())) {
